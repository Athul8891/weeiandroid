...
import 'package:weei/Screens/Main_Screens/Profile/Data/shareProfileToChat.dart';

import '../Chat/Data/verifyMessagePath.dart';


class SentWidgetProfileShare extends StatefulWidget {
  ...
  SentWidgetProfileShare({this.path,this.id,this.sendData,this.partnerData,});

  @override
  _GroceryListState createState() => _GroceryListState();
}

class _GroceryListState extends State<SentWidgetProfileShare> {
    ...
    return   loading==true? SpinKitRing(
      ...
      if(send==false){
        ...
        if(widget.partnerData['fcmToken'].toString()!="null"&&widget.partnerData['sendNoti'].toString()!="false"){
          ...
          sendPushMessage( "Shared an account 👤",  name,  widget.partnerData['fcmToken'].toString());
        }

        rsp  = await   shareProfileToMeassage(widget.path,widget.sendData,);
