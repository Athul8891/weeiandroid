...
import 'package:weei/Helper/stateManagement.dart';
import 'package:weei/Screens/voicenote/colors.dart';
import 'package:weei/Screens/voicenote/duration.dart';
import 'package:weei/Screens/voicenote/noises.dart';
import 'package:weei/Screens/voicenote/widgets.dart';
...
class VoiceMessageSession extends StatefulWidget {
  VoiceMessageSession({
    Key? key,
    required this.audioSrc,
    required this.time,
    required this.urlLauched,
    required this.me,
    required this.index,
...
class _VoiceMessageState extends State<VoiceMessageSession>
  ...
  void initState() {
    ...
    super.initState();
    //
    // widget.playingController.addListener((){
    //   //here you have the changes of your textfield
    //   print("valaaaue ${widget.playingController.text}");
    //   //use setState to rebuild the widget
    //   if(widget.playingController.text!=widget.index){
    //   //  _stopPlaying();
    //     print("stooop");
    //     print(widget.index);
    //   }
  ...
  Container _sizerChild(BuildContext context) {
    ...
    // int updatedNumber = Provider.of<MyClass>(context, listen: true) as int;
    //  print("updatedNumber");
    //  print(updatedNumber);
      return Container(
  ...
  _playButton(BuildContext context) => InkWell(
    child: Container(
      ...
      child: InkWell(
        ...
        widget.playingController.text= widget.index.toString();
  ...
  _startPlaying() async {
    var _id =widget.audioSrc.toString().split('voicenotes%').last;
    _id =_id.split('?alt').first;
    var getPath = await getSharedPrefrence(_id);

    _playingStatus = await _player.play(getPath,isLocal: true);
    _setPlayingStatus();
    _controller!.forward();
  }
  ...
  void _setDuration() async {
    ...
    if(getPath!=null){
      ...
      print("typee");
      print("locaaal");
      _audioDuration = Duration(seconds: int.parse(widget.time.toString()));

      //_audioDuration = await jsAudio.AudioPlayer().setFilePath(getPath);
    }else{
      print("typee");

      print("online");

      var file = await DefaultCacheManager().getSingleFile(widget.audioSrc);
      var save = setSharedPrefrence(_id,file.path);
  ...
  //  widget.playingController.text= widget.index.toString();
      print(widget.playingController.text);
  ...
  void _listenToRemaningTime() {
    _player.onAudioPositionChanged.listen((Duration p) {
      ...
      print("taaaaaaap");
      print(_isPlaying);
      widget.playingController.text=_isPlaying.toString();
